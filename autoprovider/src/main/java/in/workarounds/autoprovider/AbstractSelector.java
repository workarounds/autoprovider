...
public abstract class AbstractSelector<T extends AbstractSelector<?>> {
    ...
    public Uri uri() {
        Uri uri = baseUri();
        if (mNotify != null) uri = BaseProvider.notify(uri, mNotify);
        if (mGroupBy != null) uri = BaseProvider.groupBy(uri, mGroupBy);
        if (mHaving != null) uri = BaseProvider.having(uri, mHaving);
        if (mLimit != null) uri = BaseProvider.limit(uri, String.valueOf(mLimit));
        return uri;
    }
